      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          children: [
            ...
            if (user.role != null) ...[SizedBox(height: 10.h), _RoleBadge(role: user.role!)],
            if (user.isDeleted) ...[
              ...
              Container(
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    SizedBox(width: 6.w),
                    Text('Deleted Account', style: TextStyle(fontSize: 12.sp, color: AppColors.error, fontWeight: FontWeight.w600)),
                  ],
                ),
              ),
            ],
            ...
            if (user.role != null)
              _DetailRow(icon: Icons.shield_outlined, label: 'Role', value: user.role![0].toUpperCase() + user.role!.substring(1)),
            if (user.createdAt != null)
              _DetailRow(icon: Icons.calendar_today_outlined, label: 'Joined', value: _fmt(user.createdAt!)),
            if (user.updatedAt != null)
              _DetailRow(icon: Icons.update_outlined, label: 'Last Updated', value: _fmt(user.updatedAt!)),
          ],
        ),
      ),
    );
...
class _RoleBadge extends StatelessWidget {
  ...
  Color _color() {
    switch (role.toLowerCase()) {
      ...
      case 'moderator': return AppColors.secondary;
  ...
  Widget build(BuildContext context) {
    return Card(
      ...
      child: ListTile(
        ...
        title: Text(label, style: TextStyle(fontSize: 11.sp, color: AppColors.textSecondary)),
        subtitle: Text(value, style: TextStyle(fontSize: 15.sp, fontWeight: FontWeight.w500, color: Theme.of(context).colorScheme.onSurface)),
      ),
    );
  }
}
